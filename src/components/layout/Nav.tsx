...
const Nav = (): JSX.Element => {
  ...
  const totalCount = Object.values(cart.items ?? {}).reduce((sum, { count }) => sum + count, 0);

  const [isDark, setIsDark] = useState(() => localStorage.getItem("theme") === CONSTANTS.THEME.DARK);
  ...
  useEffect(() => {
    const theme = isDark ? CONSTANTS.THEME.DARK : CONSTANTS.THEME.LIGHT;
  ...
  const onToggleTheme = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    <header className="fixed z-10 w-full navbar shadow-lg bg-white dark:bg-neutral text-neutral-content">
      <div className="flex w-full xl:container xl:mx-auto">
        ...
        <div className="flex items-center px-2">
          <label className="swap swap-rotate mr-2 sm:mr-4 cursor-pointer">
            <input type="checkbox" className="js-theme" onChange={onToggleTheme} checked={isDark} />
          ...
          <div className="dropdown">
            ...
            <Search isSearchOpen={isSearchOpen} onClose={() => setIsSearchOpen(false)} />
          </div>

          <Link to="/cart" className="btn btn-ghost w-10 sm:w-12 ml-1 js-cart">
